...
- [ ] Service 事务边界正确。
- [ ] Mapper/SQL 参数绑定安全。
- [ ] 涉及 Mapper/SQL、列表、搜索、统计、导出、分页或排序时，已按 `sql-performance-review` 审查模式完成 SQL/查询接口检查。
- [ ] 关键 SQL 已提供 `EXPLAIN` 证据，或写明低风险理由。
- [ ] 列表/搜索接口有默认分页、最大 pageSize 和稳定排序。
- [ ] 已检查 N+1 查询、循环查库、大字段列表返回和 in-memory 分页/排序风险。
- [ ] 异常处理清晰。
- [ ] 日志足够排障。
- [ ] 权限校验在后端。
- [ ] 幂等/并发风险已处理。
